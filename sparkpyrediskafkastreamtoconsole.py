...
redisServerRawStreaming_df = spark \
    .readStream \
    .format("kafka") \
...
redisServerRawStreaming_df = redisServerRawStreaming_df.selectExpr("cast(value as string) value")
...
redisServerRawStreaming_df.withColumn("value", from_json("value", redisMessageSchema)) \
    .select(col("value.*")) \
...
decodedCustomer_df = encodedCustomer_df \
.withColumn("customer", unbase64(encodedCustomer_df.encodedCustomer)
            .cast("string"))

# TO-DO: parse the JSON in the Customer record and store in a temporary view called CustomerRecords
decodedCustomer_df.withColumn("customer", from_json("customer", customerSchema)) \
    .select(col("customer.*")) \
...
# /data/spark/bin/spark-submit --packages org.apache.spark:spark-sql-kafka-0-10_2.12:3.1.2 /home/workspace/sparkpyrediskafkastreamtoconsole.py
